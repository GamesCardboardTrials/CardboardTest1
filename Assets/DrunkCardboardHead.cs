...
public class DrunkCardboardHead : MonoBehaviour {
	...
	private float drunknessAmplitude = 1;
	public float drunknessFrequency;
	public BlurOptimized rightEye;
	public BlurOptimized leftEye;
	private float time;
	...
	private void UpdateHead() {
		...
		leftEye.blurSize = this.drunkness*2+this.drunknessAmplitude*Mathf.Sin(this.drunknessFrequency*time);
		rightEye.blurSize = this.drunkness*2+this.drunknessAmplitude*Mathf.Sin(this.drunknessFrequency*time);
		...
		if (trackRotation) {
			...
			if (target == null) {
				...
				float angle = Quaternion.Angle(transform.localRotation,rot);
				angle *=Mathf.PI/180f;
				transform.localRotation = Quaternion.RotateTowards(transform.localRotation, rot, angle/drunkness);
				
				
			} else {
				//transform.rotation = rot * target.rotation;
				transform.rotation = Quaternion.RotateTowards(transform.rotation, rot*transform.rotation, Mathf.PI/2f);
